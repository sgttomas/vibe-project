...
# Start Backend Framework (Port 3002)
echo -e "${BLUE}2️⃣  Starting Backend Framework (port 3002)...${NC}"
cd "../chirality-semantic-framework"
if [ ! -d "node_modules" ]; then
    echo -e "${YELLOW}📦 Installing Backend Framework dependencies...${NC}"
    npm install
fi
PORT=3002 npm run dev &
FRAMEWORK_PID=$!
PIDS+=($FRAMEWORK_PID)
echo -e "${GREEN}✅ Backend Framework started (PID: $FRAMEWORK_PID)${NC}"

# Wait for Framework to be ready
echo -e "${YELLOW}⏳ Waiting for Backend Framework to be ready...${NC}"
for i in {1..30}; do
    if curl -s http://localhost:3002/api/healthz >/dev/null 2>&1; then
        echo -e "${GREEN}✅ Backend Framework is ready${NC}"
        break
    fi
    if [ $i -eq 30 ]; then
        echo -e "${RED}❌ Backend Framework failed to start${NC}"
        exit 1
    fi
    sleep 1
done
...
echo -e "${BLUE}3️⃣  Starting Frontend App (port 3000)...${NC}"
cd "../chirality-ai-app"
if [ ! -d "node_modules" ]; then
    echo -e "${YELLOW}📦 Installing Frontend App dependencies...${NC}"
    npm install
fi
npm run dev &
APP_PID=$!
PIDS+=($APP_PID)
echo -e "${GREEN}✅ Frontend App started (PID: $APP_PID)${NC}"

# Wait for Frontend App to be ready
echo -e "${YELLOW}⏳ Waiting for Frontend App to be ready...${NC}"
for i in {1..30}; do
    if curl -s http://localhost:3000/api/healthz >/dev/null 2>&1; then
        echo -e "${GREEN}✅ Frontend App is ready${NC}"
        break
    fi
    if [ $i -eq 30 ]; then
